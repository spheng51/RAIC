...
- `corepack pnpm test tests/server/discord-integration-routes.test.ts`
  - Result: 27 tests passed, including OAuth callback state-cookie cleanup, explicit OAuth denial routing, missing-config OAuth callback feedback, recoverable connection snapshot channel-load warnings, active-organization connection snapshot/channel-save/disconnect scoping, missing-config channel-save protection, recoverable channel-save error coverage, failed-sync persisted event return, and teacher-only `GET`/`POST`/`DELETE` connection-route access coverage.
...
- `tests/lib/discord-studio-callback-hook.test.tsx` covers teacher-server success/error callback handling, one-time connection refresh, URL scrubbing that preserves unrelated query params and hashes, rerender idempotence, and public-mode no-op behavior.
