...
- `corepack pnpm test tests/server/discord-beta-smoke-script.test.ts`
  - Result: 11 tests passed after protected-preview blocker/bypass coverage, `?discord=not_configured` checklist guidance, richer response diagnostics, invalid-url regression coverage, malformed reminder-cron count coverage, and the stricter valid-Discord-event-URL sync proof were added.
