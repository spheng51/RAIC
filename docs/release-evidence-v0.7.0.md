...
- `corepack pnpm test tests/server/discord-beta-smoke-script.test.ts`
  - Result: 11 tests passed after protected-preview blocker/bypass coverage, `?discord=not_configured` checklist guidance, cron unauth smoke guarding, richer response diagnostics, invalid-url regression coverage, malformed reminder-cron count coverage, and the stricter valid-Discord-event-URL sync proof were added.
- `npx -y node@24 /usr/local/bin/corepack pnpm test tests/server/discord-beta-smoke-script.test.ts`
  - Result before the invalid-url and malformed cron-count regressions were added: 9 tests passed under Node 24. The updated 11-test slice passed locally under the available Node runtime; PR CI's Node 24 Unit Tests and Server test slice provide the release-runtime coverage for this test file.
...
- `corepack pnpm test tests/server/vercel-env-audit.test.ts`
  - Result: 11 tests passed after adding sanitized Vercel CLI JSON parsing, REST/CLI env-record sanitization coverage, script-boundary CLI fallback coverage, PATH preservation coverage, explicit API-source no-fallback coverage, and script-output secret non-leakage assertions.
