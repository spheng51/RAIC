...
- `tests/server/production-milestone-smoke-script.test.ts` covers production milestone smoke skipping Discord readiness unless required, blocking when `RAIC_REQUIRED_PRODUCTION_FEATURES=discord` while Discord env is incomplete, and writing sanitized JSON evidence with URL query secrets stripped from the normalized base URL.
